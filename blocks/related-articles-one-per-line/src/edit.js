...
class RelatedArticlesEdit extends Component {
  ...
  render () {
    ...
    const inspectorControls = (
      <InspectorControls>
        <PanelBody title={ __('Related Articles One Per Line Settings') }>
          <p>{ __('Title Level') }</p>
          <Toolbar
    ...
    if (! hasPosts) {
      return (
        <Fragment>
          ...
          <Placeholder
            icon="admin-post"
            label={ __('Related Articles One Per Line') }
          >
    ...
    return (
      <Fragment>
        { inspectorControls }
        <div className="related-articles-one-per-line articles articles--from-block articles--one-col">
          <RichText
            identifier="title"
            className="related-articles-one-per-line__title"
            tagName={ tagName }
            value={ title }
            onChange={ value => setAttributes({ title: value }) }
            formattingControls={ [] }
          />
          <div className={ `related-articles-one-per-line__items` }>
            { Array.from({ length: columns }).map((_v, i) => {
              const post = latestPosts[i];

              if (!post) {
                return <div className="related-articles-one-per-line__item" key={ i }></div>;
              }

              const titleTrimmed = post.title.rendered.trim();
              return (
                <div className="related-articles-one-per-line__item article" key={ i }>
                  <img className="article__img" src={ post.featured_media_data.source_url } alt={ post.featured_media_data.alt_text } />
                  <div className="article-info">
                    { displayPostDate && post.date_gmt &&
                    <time dateTime={ format('c', post.date_gmt) } className="related-articles-one-per-line__post-date">
                      { dateI18n(dateFormat, post.date_gmt) }
                    </time>
                    }

                    <p className="related-articles-one-per-line__item__title">
                      { titleTrimmed ? (
